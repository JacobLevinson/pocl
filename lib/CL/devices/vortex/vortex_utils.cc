...
uint32_t alignOffset3(uint32_t offset, uint32_t alignment)
{
  ...
  for (auto& OldArg : function->args()) {
    ...
    if (pocl::isLocalMemFunctionArg(function, arg_idx)) {
      if (allocated_local_mem == nullptr) {
        ...
        arg_offset = alignOffset3(arg_offset + 4, BaseAlignment);
      ...
      arg_offset = alignOffset3(arg_offset + DL.getTypeAllocSize(ArgType), BaseAlignment);
    }
