...
uint32_t alignOffset4(uint32_t offset, uint32_t alignment)
{
...
void pocl_vortex_run (void *data, _cl_command_node *cmd) {
  ...
  uint32_t aligned_kernel_args_size = alignOffset4(sizeof(kernel_args_t), ptr_size);
  ...
  for (int i = 0; i < meta->num_args; ++i) {
    ...
    if (ARG_IS_LOCAL(meta->arg_info[i])) {
      ...
      abuf_size = alignOffset4(abuf_size + 4, ptr_size);
    } else
    if ((meta->arg_info[i].type == POCL_ARG_TYPE_POINTER)
     || (meta->arg_info[i].type == POCL_ARG_TYPE_IMAGE)
     || (meta->arg_info[i].type == POCL_ARG_TYPE_SAMPLER)) {
      abuf_size = alignOffset4(abuf_size + ptr_size, ptr_size);
    } else {
      // scalar argument
      abuf_size = alignOffset4(abuf_size + al->size, ptr_size);
    }
  }
  ...
  for (int i = 0; i < meta->num_args; ++i) {
    ...
    if (ARG_IS_LOCAL(meta->arg_info[i])) {
      if (local_mem_offset == 0) {
        ...
        host_args_offset = alignOffset4(host_args_offset + 4, ptr_size);
      }
    ...
    if (meta->arg_info[i].type == POCL_ARG_TYPE_POINTER) {
      if (al->value == NULL) {
        ...
        host_args_offset = alignOffset4(host_args_offset + ptr_size, ptr_size);
      } else {
    ...
    if (meta->arg_info[i].type == POCL_ARG_TYPE_SAMPLER) {
      ...
      host_args_offset = alignOffset4(host_args_offset + al->size, ptr_size);
    }
  }
